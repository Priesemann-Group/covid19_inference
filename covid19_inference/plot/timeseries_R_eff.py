...
def timeseries_R_eff(
    ...
    -------
        fig : mpl figure
        axes : np array of mpl axes (insets not included)
    ...
    * Replace `offset` with an instance of data class that should yield the
    ...
    label_leg_data = "Data"
    label_leg_dlim = f"Inference for Germany\nin the year 2020"
    ...
    R_eff, x = get_array_from_idata_via_date(model, idata, "R_eff")
    y = R_eff[:, :, region]
    ...
    for ax in axes:
        ...
        # regulate number of xtick labels
        if not axes_provided:
